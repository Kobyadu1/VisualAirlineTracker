...
/**
 * Calculates the bearing between Los Angeles and a passed in secondary city
 */ 
public class HeadingCalculationLAX extends HeadingCalculationAbstract{
  double bearing;
  //Default constructor
  public HeadingCalculationLAX() {
    super();
    bearing = 0;
  }
  //Constructor that runs the bearing calculation
  public HeadingCalculationLAX(String airportCode) {
    super(airportCode);
    bearing = calculateBearing();
  }
  public HeadingCalculationLAX(Double lat,Double lon) {
    super(lat,lon);
    bearing = calculateBearing();
  }
  ...
  public double calculateInputOne() {
    ...
    double returnValue;
    lonDifference = getAirportCords()[1]-getLOSANGELESCORDS()[1];
  ...
  public double calculateInputTwo() {
    ...
    firstArguement = Math.cos(Math.toRadians(getLOSANGELESCORDS()[0])) * Math.sin(Math.toRadians(getAirportCords()[0]));
    secondArguement = Math.sin(Math.toRadians(getLOSANGELESCORDS()[0])) * Math.cos(Math.toRadians(getAirportCords()[0])) *  Math.cos(Math.toRadians(lonDifference));
