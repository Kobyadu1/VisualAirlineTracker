...
/**
 * Calculates the bearing between Chicago and a passed in secondary city
 */ 
public class HeadingCalculationORD extends HeadingCalculationAbstract{
  double bearing;
  //Default constructor
  public HeadingCalculationORD() {
    super();
    bearing = 0;
  }
  //Constructor that runs the bearing calculation
  public HeadingCalculationORD(String airportCode) {
    super(airportCode);
    bearing = calculateBearing();
  }
  public HeadingCalculationORD(Double lat,Double lon) {
    super(lat,lon);
    bearing = calculateBearing();
  }
  ...
  public double calculateInputOne() {
    ...
    double returnValue;
    lonDifference = getAirportCords()[1]-getCHICAGOCORDS()[1];
  ...
  public double calculateInputTwo() {
    ...
    firstArguement = Math.cos(Math.toRadians(getCHICAGOCORDS()[0])) * Math.sin(Math.toRadians(getAirportCords()[0]));
    secondArguement = Math.sin(Math.toRadians(getCHICAGOCORDS()[0])) * Math.cos(Math.toRadians(getAirportCords()[0])) *  Math.cos(Math.toRadians(lonDifference));
